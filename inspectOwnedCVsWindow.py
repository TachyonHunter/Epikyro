...
def InspectOwnedCVsWindow(user):
    ...
    if OwnedCVs == 'not found':
        ttk.Label(mainframe, text='No CVs associated with this user', style='Sub-headings.TLabel', justify='left').grid(row=0, column=0, sticky='W', padx=8)

    else:
        canvasFrame = ttk.Frame(mainframe)
        canvasFrame.grid(row=1, column=0, sticky='NSEW')

        def EditInteractiveFrame(interactiveElementsFrame, employeeName):
            ttk.Button(interactiveElementsFrame, text=f'Open CV', style='Buttons.TButton').grid(row=0, column=0)
        ...
        ttk.Label(mainframe, text=f'{user}\'s owned CVs:', justify='left', style='Headings.TLabel').grid(row=0, column=0, sticky='W', padx=8)
